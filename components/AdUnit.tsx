...
const AdUnit: React.FC<AdUnitProps> = ({ 
  ...
  useEffect(() => {
    try {
      // Check if window.adsbygoogle exists, then push to it.
      // This triggers the ad script to scan for the <ins> tag.
  ...
  return (
    <div className={`ad-container my-8 overflow-hidden rounded-xl bg-slate-100 dark:bg-slate-900 flex justify-center items-center min-h-[100px] ${className}`}>
      {/* 
        NOTE: You must replace 'ca-pub-XXXXXXXXXXXXXXXX' below with your actual publisher ID 
        if you want it hardcoded, or rely on the script in index.html to set it globally.
        However, standard practice is to include data-ad-client here too.
      */}
      <ins
