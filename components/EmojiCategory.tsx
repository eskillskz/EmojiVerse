...
const EmojiCategory: React.FC<EmojiCategoryProps> = ({ 
  ...
  return (
    <section 
      id={id}
      className="mb-4 sm:mb-6 bg-white dark:bg-slate-900/40 backdrop-blur-sm border border-slate-200 dark:border-white/5 rounded-3xl shadow-sm dark:shadow-lg transition-all duration-300 hover:shadow-md dark:hover:bg-slate-900/60 dark:hover:border-white/10 hover:dark:shadow-2xl"
    >
      {/* Accordion Header */}
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center justify-between px-4 sm:px-6 py-4 sm:py-5 bg-transparent transition-colors cursor-pointer group rounded-3xl focus:outline-none"
      >
        <div className="flex items-center gap-3 sm:gap-4 overflow-hidden">
          ...
          {/* Preview when closed */}
          {!isOpen && (
            <div className="flex items-center gap-0.5 sm:gap-1 opacity-60 group-hover:opacity-100 transition-opacity overflow-hidden">
              {previewEmojis.map((e) => (
                <span key={e.hexcode} className="text-lg sm:text-xl transition-all hover:scale-125">{e.emoji}</span>
              ))}
        ...
        <div className={`p-2 rounded-full bg-slate-100 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 transition-transform duration-300 group-hover:text-slate-800 dark:group-hover:text-white ${isOpen ? 'rotate-180' : ''}`}>
          <ChevronDown size={20} />
        </div>
      </button>
      ...
      >
        <div className="p-3 sm:p-6 pt-0">
          {/* Grid density increased for mobile: grid-cols-8 instead of 5 */}
          <div className="grid grid-cols-8 sm:grid-cols-8 md:grid-cols-10 lg:grid-cols-12 gap-1 sm:gap-3">
